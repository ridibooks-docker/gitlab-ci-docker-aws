...
ENV NODEJS_VERSION=v10.15.1 NPM_VERSION=6 YARN_VERSION=latest
ENV CONFIG_FLAGS="--fully-static --without-npm" DEL_PKGS="libstdc++" RM_DIRS=/usr/include

RUN apk add --no-cache gcc g++ linux-headers binutils-gold gnupg libstdc++ && \
  ...
  curl -sfSL https://nodejs.org/dist/${NODEJS_VERSION}/SHASUMS256.txt.asc | gpg -d -o SHASUMS256.txt && \
  grep " node-${NODEJS_VERSION}.tar.xz\$" SHASUMS256.txt | sha256sum -c | grep ': OK$' && \
  ...
  rm -rf ${RM_DIRS} /node-${NODEJS_VERSION}* /SHASUMS256.txt /usr/share/man /tmp/* /var/cache/apk/* \
